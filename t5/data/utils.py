...
class Feature(object):
  ...
  def __init__(self, add_eos=True, vocabulary=None):
    ...
    if vocabulary is None:
      raise ValueError("A vocabulary must be provided when creating a Feature.")
  ...
  def __init__(self,
    ...
    Args:
      ...
      sentencepiece_model_path: string, path to a SentencePiece model file to
        use for tokenization when a `Feature` in `output_features` is not
        supplied which has a vocabulary defined. For provided `Feature`s in
        `output_features`, this argument will be ignored.
  ...
  def get_dataset(
    ...
    if use_cached:
      ...
      ds = encode_string_features(
          ds, self._output_features, keys=self.output_features,
          copy_plaintext=True)
